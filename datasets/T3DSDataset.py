...
class T3DS(data.Dataset):
    ...
    def __init__(self, config):
        ...
        self.n_renderings = 3 if self.subset == 'train' else 3
    ...
    def _get_file_list(self, subset, n_renderings=1):
        ...
        for dc in self.dataset_categories:
            print_log('Collecting files of Taxonomy [ID=%s, Name=%s]' % (dc['taxonomy_id'], dc['taxonomy_name']), logger='T3DSDATASET')
            samples = dc[subset]
        ...
        print_log('Complete collecting files of the dataset. Total files: %d' % len(file_list), logger='T3DSDATASET')
        return file_list
